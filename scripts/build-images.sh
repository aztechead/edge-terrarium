...
# Unified smart build script for Edge-Terrarium Docker images
# Supports both Docker Compose and K3s environments with smart caching
...
if [ "$1" = "--k3s" ]; then
    ...
    echo "Building Edge-Terrarium Docker images with smart caching..."
fi
...
# Check if we're in K3s environment (only when K3s mode is enabled)
if [ "$K3S_MODE" = true ] && ! docker info | grep -q "k3s"; then
...
# Build core services (common to both environments)
...
# Environment-specific success messages and instructions
if [ "$K3S_MODE" = true ]; then
    print_success "All images built successfully for K3s environment!"
    echo ""
